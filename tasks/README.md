...
This directory is both Tsk's own build plan and an example of its format. `project.ason` identifies it as a Tsk task directory. Each ID has a flat directory containing `task.ason` and, if needed, artifacts. IDs are short, lowercase Crockford base32 (`0123456789abcdefghjkmnpqrstvwxyz`); don't infer dependencies from their order. The directory name is the ID.
