...
test('done updates a task from a nested directory and preserves extra fields and attachments', async () => {
	...
	expect(parse(result.stdout.toString())).toEqual({ title: 'Task b', description: 'Build b', status: 'done', needs: ['a'], extra: 'keep me' })
	expect(result.stdout.toString()).toContain('// a note')
